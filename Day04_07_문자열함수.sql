...
SELECT 
        UPPER(EMAIL)    -- 대문자
      , LOWER(EMAIL)    -- 소문자
      , INITCAP(EMAIL)  -- 첫 글자만 대문자, 나머지는 소문자 변환
  FROM
        EMPLOYEES;
...
-- 4. 문자열의 일부 반환하기
--    SUBSTR(칼럼, BEGIN, LENGTH) : BEGIN 부터 LENGTH개 -- 어디서부터 몇 개
--    주의! BEGIN은 1에서 시작한다.
SELECT 
...
--    2) RPAD(칼럼, 전체폭, 채울문자)
SELECT
        LPAD(NVL(DEPARTMENT_ID, 0), 3, '0')     -- DEPARTMENT_ID가 NULL이 나오면 0이라고 할게
