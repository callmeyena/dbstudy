/*
    ...
    3. 형식
        CREATE [OR REPLACE] PROCEDURE 프로시저명 [(매개변수)] 매개변수 괄호 자체가 생략될 수 있음
        IS -- AS도 가능
             변수선언                                       
        BEGIN
            본문
        [EXCEPTION
            예외처리]
        END;
    4. 작성 된 프로시저는 EXECUTE 프로시저(); 형식으로 실행한다. / 실행할 때는 괄호 붙여야함
*/

-- 프로시저 1 정의(만들기)
CREATE OR REPLACE PROCEDURE PROC1   -- 매개변수 없는 형태
IS
BEGIN
...
-- 프로시저5. 사원번호를 전달하면, 해당 사원의 FIRST_NAME이 출력 파라미터 FNAME에 저장되도록 저장하시오
-- 없는 사원번호가 전달되면 출력 파라미터 FNAME에 'NONAME'이 저장되도록 작성.
-- 실행 예시)
-- PROC5(100, FNAME): 사원번호 100은 Steven입니다.
-- PROC5(0, NONAME): 사원번호 0은 NONAME입니다.
