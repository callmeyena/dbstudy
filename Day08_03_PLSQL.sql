...
/*
    ...
    3. 복사하는 쿼리
        1) 데이터를 복사하기
            CREATE TABLE 테이블명 AS(SELECT 칼럼명 FROM 테이블명);
        2) 데이터를 제외하고 구조만 복사하기
            CREATE TABLE 테이블명 AS(SELECT 칼럼명 FROM 테이블명 WHERE 1=2);   -- 1=2조건이 언제나 FALSE이기 때문에 데이터는 복사되는 게 없고 칼럼들만 복사하겠다는 뜻
*/
-- HR계정의 EMPLOYEES 테이블을 GDJ61 계정으로 복사해서 기초 데이터로 사용하기
DROP TABLE EMPLOYEES;
CREATE TABLE EMPLOYEES
    AS(SELECT *
         FROM HR.EMPLOYEES);    -- 테이블 복사할 때 사용하는 서브 쿼리
...
-- EMPLOYEE_ID가 150인 사원의 COMMISSION_PCT가 0이면 '커미션 없음', 아니면
DECLARE
    EMP_ID EMPLOYEES.EMPLOYEE_ID%TYPE;
...
-- 사원번호 100 ~ 206인 사원들의 FIRST_NAME , LAST_NAME 을 조회하시오
DECLARE
    EMP_ID EMPLOYEES.EMPLOYEE_ID%TYPE;
    FNAME EMPLOYEES.FIRST_NAME%TYPE;
    LNAME EMPLOYEES.LAST_NAME%TYPE;
    ...
    EMP_ID := 100;
     WHILE EMP_ID <= 206 LOOP
        SELECT FIRST_NAME, LAST_NAME
          ...
          FROM EMPLOYEES
         WHERE EMPLOYEE_ID = EMP_ID;
      DBMS_OUTPUT.PUT_LINE(FULL_NAME.FNAME || ' ' || FULL_NAME.FNAME);
      EMP_ID := EMP_ID + 1;
    END LOOP;
END;
...
-- 3) DEPARTMENT_ID가 50이면 행 변수에 저장된 내용을 DEPT 50 테이블에 INSERT
...
-- 1 부터 3의 배수를 제외한 정수 값을 누적하시오. 누적 값이 100을 초과하면 그만누적하고 어디까지 누적했는지 출력하시오
DECLARE
    N NUMBER;
    TOTAL NUMBER;
    MODULAR NUMBER(1);
...
EXCEPTION
    WHEN NO_DATA_FOUND THEN
        DBMS.OUTPUT.PUT_LINE('조회된 데이터가 없습니다.');
END;
