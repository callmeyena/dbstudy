/*
    사용자 함수(FUNTION)
    1. 어떤 값을 반환 할 때 사용하는 데이터베이스 객체이다.
    2. 실제로 함수를 만들어서 사용하는 개념이다.
    3. RETURN 개념이 존재한다.
    4. 함수의 결과 값을 확인할 수 있도록 SELECT문에서 많이 사용한다.
    5. 형식
        CREATE [OR REPLACE] FUNCTION 함수명[(매개변수)] 매개변수가 없을 때는 괄호까지 생략
        RETURN 반환타입
        IS -- AS도 가능
            변수 선언
        BEGIN
...
-- 사용자 함수 FUNC2 정의
-- 사원번호를 전달하면 해당 사원의 FULL_NAME을 반환하는 함수
...
END;
 ...
 -- 사용자 함수 FUNC4 호출
SELECT EMPLOYEE_ID, FIRST_NAME, LAST_NAME, SALARY, FUNC4(EMPLOYEE_ID)   -- FUNC4(매개변수) 매개변수 값에 EMPLOYEE_ID 를 넣으면 전체 사원 조회 가능
  FROM EMPLOYEES;
...
-- 함수 MY_CEIL 호출
SELECT
        MY_CEIL(111.111, 2)     -- 소수 2자리로 올림
      , MY_CEIL(111.111, 1)     -- 소수 1자리로 올림
...
END; 
  ...
  -- 함수 MY_FLOOR 호출
SELECT 
        MY_FLOOR(111.111, 2)    -- 소수 2자리로 내림
      , MY_FLOOR(111.111, 1)    -- 소수 1자리로 내림
