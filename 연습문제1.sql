...
ALTER TABLE MEMBER_TBL ADD ARGIST_DATE DATE;
...
-- ALTER TABLE BOARD_TBL DROP CONSTRAINT SYS_C007446;
/*
SELECT *
...
ALTER TABLE BOARD_TBL
    ADD CONSTRAINT PK_BOARD_TBL PRIMARY KEY(BOARD_NO);

-- 11. 작성자 칼럼에 MEMBER_TBL 테이블의 회원아이디를 참조하는 FK_BOARD_MEMBER 외래키를 설정하시오.
-- 게시글을 작성한 회원 정보가 삭제되면 해당 회원이 작성한 게시글도 모두 함께 지워지도록 처리하시오.
ALTER TABLE BOARD_TBL
    ADD CONSTRAINT FK_BOARD_MEMBER FOREIGN KEY(WRITER) -- ADD CONSTRAINT FK_참조된테이블명_참조할테이블명 FOREIGN KEY(참조 될 칼럼명)
        REFERENCES MEMBER_TBL(ID)                      -- REFERENCES 참조할테이블명(참조할칼럼명)
        ON DELETE CASCADE;                             -- PK키가 삭제 되면 관련된 모든 정보들 삭제하겠다
