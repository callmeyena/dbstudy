/*
    ...
    2. 데이터 조작어
    3. 데이터(행, Row)를 삽입, 수정, 삭제 할 때  사용하는 언어이다.
...
*/
-- 참고! 간혹 자격증에서는 DML을 INSERT, UPDATE, DELETE + SELECT로 보는 경우도 있다.
...
-- 시퀀스(번호 생성기) 만들기  => 보통 PK만들 때 사용함
CREATE SEQUENCE DEPARTMENT_SEQ -- 기계 생성 완 .NEXTVAL 하면 1나오고 순서대로 +1 되는 것
    INCREMENT BY 1      -- 1씩 증가하는 번호를 만든다.(생략 가능)
    START WITH 1        -- 번호를 1부터 시작한다.(생략 가능)
    ...
    NOCACHE             -- 메모리 캐시를 사용하지 않는다.   |  CACHE: 메모리 캐시를 사용한다.(사용하지 않는 것이 좋다) like 미리 번호표를 뽑아놓는 것
    ORDER               -- 번호 건너뛰기가 없다.           |   NOORDER: 번호 건너뛰기 가능
;
-- 시퀀스에서 번호 뽑는 함수 : NEXTVAL
-- SELECT DEPARTMENT_SEQ.NEXTVAL FROM DUAL;    -- 오라클에서는 테이블에 없는 데이터를 조회 하려면 DUAL 이라는 이름의 테이블을 사용한다.

-- 데이터 입력하기(Parent Key를 먼저 입력해야 한다.)
INSERT INTO DEPARTMENT_TBL VALUES(DEPARTMENT_SEQ.NEXTVAL, '영업부', '대구'); -- 칼럼의 이름 생략 가능
INSERT INTO DEPARTMENT_TBL(DEPT_NO, DEPT_NAME, LOCATION) VALUES(DEPARTMENT_SEQ.NEXTVAL, '인사부', '서울'); -- 이 방법으로 픽 ㅋ 칼럼의 입력 순서대로 적고, VALUES 값 잘 주기
...
-- 2. 부서번호(DEPART)가 1인 부서에 근무하는 사원들의 급여를(SALARY) 500,000원을 증가시키시오.
UPDATE EMPLOYEE_TBL
 ...
 DELETE FROM DEPARTMENT_TBL WHERE LOCATION = '대구'; -- (대구에서 근무하는 사원이 없기 때문에 삭제에 무리가 없다.)
 COMMIT;
 
 -- 2. 지역(LOCATION)이 '서울'인 부서를 삭제하시오. | EMPLOYEE 외래키 설정에서 ON DELETE SET NULL; 이라고 우리가 지정을 해줬기 때문에 사원은 남아있고 널값이 들어감
 DELETE FROM DEPARTMENT_TBL WHERE LOCATION = '서울'; -- 서울에서 근무하는 ON DELETE SET NULL 외래키 옵션에 의해서 NULL로 처리된다.)
 COMMIT; 
