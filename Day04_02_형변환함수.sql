/*
    ...
    2. 아무 일도 안하는 테이블이다.
    3. 오라클은 SELECT TO FROM절이 필수이므로 언제나 테이블이 필요하다.
        테이블이 필요 없는 값을 조회할 때 DUAL테이블을 사용한다.
*/
...
SELECT EMPLOYEE_ID, FIRST_NAME, LAST_NAME
  FROM EMPLOYEES
 WHERE EMPLOYEE_ID = '150';         -- 숫자와 '문자'의 동등비교 연산 => 문자(TO_NUMBER)에서 숫자로 자동 변환 된다. EX) EMPLOYEE_ID = TO_NUMBER('150')으로 자동으로 변환한 뒤 처리한다.
 
 /* 2. 문자로 변환하기(TO_CHAR) => 날짜에서 제일 많이 사용한다. */
 ...
 -- 2023-02-07 => '2023년 02월 07일  -- 숫자와 날짜 로 바라볼 ....
...
/* 3. 날짜로 변환하기(TO_DATE) */
-- 날짜와 시간을 원하는 형식으로 표시하는 함수가 아님을 주의 하기
