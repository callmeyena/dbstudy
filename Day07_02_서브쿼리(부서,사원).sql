...
/*
    서브쿼리의 실행 결과에 의한 구분
    1. 단일 행 서브 쿼리(SINGLE ROW SUB QUERY)
        1) 결과 행이 1개다.
...
SELECT 사원정보
 ...
 SELECT EMP_NO, NAME, DEPART, GENDER, POSITION, HIRE_DATE, SALARY
  ...
  WHERE POSITION = (SELECT POSITION
                      FROM EMPLOYEE_TBL
                     WHERE EMP_NO = 1001);      -- 서브커리에 PK(EMP_NO)가 WHERE절에 사용 됐기 때문에 단일 행이 100% 맞다
...
SELECT 부서정보
 ...
 SELECT DEPT_NO, DEPT_NAME, LOCATION
   FROM DEPARTMENT_TBL
  WHERE LOCATION = (SELECT LOCATION         -- 메인쿼리에서 LOCATION 이면 서브쿼리(SELECT)에서도 LOCATION
                      FROM DEPARTMENT_TBL
                     WHERE DEPT_NO = 2);
...
SELECT 사원정보
  FROM 사원
 WHERE 직급 IN (부서 번호가 2인 부서에 근무하는 사원들의 직급);
...
/*
    인라인 뷰(Inline view(=table)) => 쿼리문 내부로 들어온 새로운 쿼리문
...
/*
    ...
    4. 종류
        ...
        2) ROWNUM: 행(ROW) 번호, 어떤 행의 순번  -- 오후 수업의 목적 ㅋ 중요중요짱중요
*/
...
--  실무에선 ROWID 사용 불가능하기 때문에 대신 인덱스(INDEX)를 활용한다.
SELECT EMP_NO, NAME
  FROM EMPLOYEE_TBL
 WHERE ROWID = 'AAAFBHAABAAALDBAAB';
  
/*
    ROWNUM의 제약 사항
    1. 행번호(ROWNUM)가 1을 포함하는 범위를 조건으로 사용할 수 있다.
    2. 행번호(ROWNUM)가 1을 포함하지 않는 범위를 조건으로 사용할 수 없다.
    3. 모든 행번호(ROWNUM)를 사용하려면 ROWNUM에 별명을 지정하고 그 별명을 사용하면 된다.
*/
SELECT EMP_NO, NAME 
  FROM EMPLOYEE_TBL
...
SELECT EMP_NO, NAME 
 ...
 SELECT ROWNUM AS RN, ENP_NO, NAME
  ...
  WHERE RN = 2;         -- 실행 순서가 맞지 않기 때문에 실행이 불가능하다.(별명을 사용할 수 없다.)
                        -- 해결방법: 별명 지정을 WHERE보다 먼저 처리하면 해결된다. => 인라인 뷰 => 가장 먼저 동작
...
--  2) ROW_NUMBER() 함수 사용하기(정렬하기 + 행번호 붙이기를 한번에 주는 함수)
SELECT E.EMP_NO, E.NAME, E.DEPART, E.GENDER, E.POSITION, E.HIRE_DATE, E.SALARY
