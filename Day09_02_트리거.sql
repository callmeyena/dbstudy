...
SET SERVEROUTPUT ON;    -- DBMS의 메시지를 출력(?)하려면 하루에 한번은 켜야함
...
-- TRIG1 동작 확인(EMPLOYEES 테이블의 데이터를 수정해본다.) / 트리거는 기본적으로 업데이트 될 때 동작하는 걸로 되어있음
UPDATE EMPLOYEES
   SET LAST_NAME = 'Queen'
...
-- REITER_TRIG 동작 확인
DELETE FROM EMPLOYEES WHERE EMPLOYEE_ID BETWEEN 101 AND 105;
