...
-- 1. NVL(NULLVALUE: 칼럼, NULL대신 사용할 값)
-- NULL처리 필요한 이유는 모든 연산 결과에 NULL이 들어가면 NULL처리 되기 때문이다..!
SELECT 
        NVL(NAME, '아무개') AS 이름
      , NVL(KOR, 0) + NVL(ENG, 0) + NVL(MAT,
      0) AS 총점
  FROM
        SAMPLE_TBL;
        
-- 2. NVL2(칼럼, NULL이 아닐 때 사용할 값, NULL일 때 사용할 값)  IF를 닮았다,,,삼항 연산자를 닮았다,,,
SELECT 
