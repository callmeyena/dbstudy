/*
    ...
    4. 완료된 작업을 취소할 수 없다.(COMMIT(저장)할 필요가 없다. ROLLBACK(취소)을 할 수 없다.) => 작업하면 끝이라구~!
    5. 종류
        1) CREATE: 생성
        2) ALTER : 수정 EX) 완성된 테이블에 이름이나, 타입을 바꾸겠다, 제약조건 바꾸겠다 등
        3) DROP  : 삭제
*/
-- 테이블 삭제
...
CREATE TABLE CUSTOMER_TBL (
    ...
    CONSTRAINT FK_CUSTOM_BANK FOREIGN KEY(BANK_CODE) REFERENCES BANK_TBL(BANK_CODE)
);
...
-- 9. CUMSTOMER_TBL 테이블의 CUST_NAME 칼럼의 NOT NULL 제약조건을 NULL 제약조건으로 변경하시오.
ALTER TABLE CUSTOMER_TBL
    MODIFY CUST_NAME VARCHAR2(30 BYTE) NULL; -- 수정할 때에는(MODIFY) 제약조건을(EX) NULL) 따로 적어주지 않으면 그건 이전 제약조건을 그대로 쓰겠다는 뜻! (NULL 명시를 반드시 해야한다.)