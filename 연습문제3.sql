...
CREATE TABLE USERS (
    ...
    USER_ID VARCHAR2(20 BYTE) NOT NULL UNIQUE,  -- 사용자아이디       -- 기본키가 아니더라도 UNIQUE 처리 되어 있다면 외래키 참조 사용 가능
...
INSERT INTO USERS VALUES (USER_SEQ.NEXTVAL, 'KJD', '김제동', 1974, '경남', NULL, NULL, '13/03/03'); -- NULL: 아무것도 넣지 말고 빈칸으로 주십시오
...
DELETE 
  FROM BUYS
 -- WHERE USER_ID = (사용자번호가 5번인 사용자(USER_ID)를 따로 가져오기); -- USER_ID를 참조했기 때문에 USER_NO = 5인 사용자를 알기 위해 서브쿼리 짜야함
...
DELETE 
 ...
 WHERE USER_NO = 5;
 COMMIT;
 -- ON DELETE CASCADE 옵션이 있었다면 구매내역 지울 필요 없음


-- 4. 연락처1이 없는 사용자의 사용자번호, 아이디, 연락처1, 연락처2를 조회하시오.
SELECT USER_NO, USER_ID, USER_MOBILE1, USER_MOBILE2
  FROM USERS
 WHERE USER_MOBILE1 IS NULL; -- 빈칸이다. 값이 없다 => IS NULL 외워 그냥 예나야


-- 5. 연락처2가 '5'로 시작하는 사용자의 사용자번호, 아이디, 연락처1, 연락처2를 조회하시오.
SELECT USER_NO, USER_ID, USER_MOBILE1, USER_MOBILE2
  FROM USERS
 WHERE USER_MOBILE2 LIKE '%5';          -- 5로 시작한다 = > %5 + 등호(=)사용 안되고 LIKE 붙여야함 LIKE 연산도 같이 체크


-- 6. 제품을 구매한 사용자의 아이디별 구매횟수를 조회하시오.   => KEY WORD: 아이디별(EX) 부서별,...) => 아 동일한 아이디는 하나로 모으면 되겠구나 => GROUP BY/ 구매횟수 => COUNT
-- 아이디  구매횟수
-- KHD     3
-- PSH     3
-- KYM     1
-- LHJ     2
SELECT USER_ID AS 아이디, COUNT(*) AS 구매횟수         -- SELECT절에서 USER_ID를 조회하고 싶다면 GROUP BY에서는 USER_ID로 정해진거임
  FROM BUYS
 GROUP BY USER_ID;



-- 7. 제품을 구매한 이력이 있는 고객의 (아이디(BUYS), 고객명 => JOIN), (구매횟수, 총구매액 => GROUP BY)을 조회하시오.   -- 구매 테이블 참고하면 됨 / JOIN과 GROUP BY
-- 아이디  고객명  구매횟수  총구매액
-- LHJ     이휘재  2         80
-- KYM     김용만  1         200
-- KHD     강호동  3         1210
-- PSH     박수홍  3         1860
/* SELECT B.USER_ID AS 아이디, U.USER_NAME AS 고객명, COUNT(*) AS 구매횟수, SUM(P.PROD_PRICE * B.BUY_AMOUNT) AS 총구매액
  FROM USERS U INNER JOIN BUYS B
    ON U.USER_ID = B.USER_ID INNER JOIN PRODUCTS P
    ON P.PROD_CODE = B.PROD_CODE
 GROUP BY B.PROD_CODE, B.BUY_AMOUNT; 
*/
SELECT U.USER_ID AS 아이디
     , U.USER_NAME AS 고객명
     , COUNT(*) AS 구매횟수
     , SUM(P.PROD_PRICE * B.BUY_AMOUNT) AS 총구매액   -- 무슨제품이 얼마이고, 가격이 얼마인지  구매액(가격 * 총갯수)
  FROM USERS U INNER JOIN BUYS B
    ON U.USER_ID = B.USER_ID INNER JOIN PRODUCTS P
    ON P.PROD_CODE = B.PROD_CODE
 GROUP BY U.USER_ID, U.USER_NAME;
-- USER_ID + USER_NAME 같이 묶을거니까 GROUP BY


-- 8. 구매 이력과 상관 없이 고객별 구매횟수를 조회하시오.
--    구매 이력이 없는 경우(외부조인) 구매횟수는 0으로 조회하시오.
...
SELECT U.USER_NAME AS 고객명
     , P.PROD_NAME AS 제품명
     , SUM(P.PROD_PRICE * B.BUY_AMOUNT) AS 구매액
  FROM USERS U INNER JOIN BUYS B
    ON U.USER_ID = B.USER_ID INNER JOIN PRODUCTS P
...
-- 이휘재  2
-- 강호동  3
SELECT U.USER_NAME AS 고객명, COUNT(B.BUY_AMOUNT) AS 구매횟수
  FROM USERS U INNER JOIN BUYS B
    ON U.USER_ID = B.USER_ID
 WHERE B.BUY_AMOUNT >= 2
 GROUP BY U.USER_NAME, B.BUY_AMOUNT;
...
-- 12. 어떤 고객(USERS)이 어떤 제품(PRODUCTS)을 구매(BUYS)했는지 조회하시오.
--     구매 이력이 없는 고객도 조회(OUTER JOIN)되도록 처리하고 고객아이디로 오름차순 정렬하시오.
-- 고객명   구매제품
-- 강호동   운동화
-- 강호동   노트북
-- 강호동   청바지
...
-- 스칼라 서브쿼리로 풀어보기
SELECT U.USER_NAME AS 고객명
     , (SELECT PROD_NAME                    -- 메인쿼리와 관계 맺고 있는 서브쿼리
          FROM PRODUCTS P
         WHERE P.PROD_CODE = B.PROD_CODE) AS 제품명  -- PRODUCTS 테이블에서 P.P_CODE와 B.P_CODE가 일치한다면 SELECT에서 PROD_NAME 조회 하기 
  FROM USERS U LEFT OUTER JOIN BUYS B
    ON U.USER_ID = B.USER_ID
 ORDER BY U.USER_ID;